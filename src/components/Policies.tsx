...
const POLICIES = [
  ...
  { icon: ShieldCheckIcon, title: "Không ép giá", desc: "Định giá minh bạch trước mặt khách hàng, mua đúng giá trị thực tế của máy.", img: "/img/THU-MUA-GIA-CAO-3.jpg" },
  { icon: LockClosedIcon, title: "Bảo mật dữ liệu", desc: "Hỗ trợ xoá dữ liệu miễn phí trước khi thu mua, bảo mật tuyệt đối.", img: "/img/THU-MUA-GIA-CAO-2.jpg" },
      ...
      <div className="w-full max-w-[1300px] mx-auto px-5 sm:px-8">
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button onClick={() => scroll("left")} disabled={active === 0} className="w-9 h-9 rounded-full border border-zinc-200 flex items-center justify-center hover:bg-zinc-50 disabled:opacity-30 transition-all">
              <ChevronLeftIcon className="w-4 h-4 text-zinc-600" strokeWidth={2.5} />
            </button>
            <button onClick={() => scroll("right")} disabled={active === POLICIES.length - 1} className="w-9 h-9 rounded-full border border-zinc-200 flex items-center justify-center hover:bg-zinc-50 disabled:opacity-30 transition-all">
        ...
        <div className="hidden sm:block">
          ...
          >
            {POLICIES.map((p) => (
              <div key={p.title} className="shrink-0 w-[320px] lg:w-[360px] snap-start rounded-2xl overflow-hidden border border-zinc-200 group">
                <div className="aspect-[4/3] overflow-hidden">
                  <Image src={p.img} alt={p.title} width={360} height={270} className="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-500" />
                </div>
          ...
          <div className="flex justify-center gap-1.5 mt-5">
            {POLICIES.map((_, i) => (
              <div key={i} className={`h-1.5 rounded-full transition-all duration-200 ${i === active ? "w-6 bg-amber-500" : "w-1.5 bg-zinc-200"}`} />
            ))}
          </div>
        </div>
      </div>
